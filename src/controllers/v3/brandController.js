const { BrandV3 } = require("../../models/v3");
...
exports.getBrands = async (req, res) => {
  try {
    ...
    const brands = await BrandV3.find(filter)
      .sort(sort)
      .skip(skip)
      .limit(Number(limit));

    const total = await BrandV3.countDocuments(filter);
...
exports.getBrandById = async (req, res) => {
  try {
    const brand = await BrandV3.findById(req.params.id);
...
exports.getBrandBySlug = async (req, res) => {
  try {
    const brand = await BrandV3.findOne({
...
exports.updateBrand = async (req, res) => {
  try {
    ...
    const brand = await BrandV3.findByIdAndUpdate(
      brandId,
      brandData,
      { new: true }
    );
...
exports.deleteBrand = async (req, res) => {
  try {
    const brand = await BrandV3.findByIdAndDelete(req.params.id);
