...
exports.register = async (req, res) => {
  try {
    ...
    const existingUser = await UserV2.findOne({ email });
    if (existingUser) {
...
exports.getProfile = async (req, res) => {
  try {
    const user = await UserV2.findById(req.user.userId)
      .populate("addresses")
      .select("-password");
...
exports.getUsers = async (req, res) => {
  try {
    ...
    const users = await UserV2.find(filter)
      .select("-password")
      .sort(sort)
      .skip(skip)
      .limit(Number(limit));

    const total = await UserV2.countDocuments(filter);
...
exports.getUserById = async (req, res) => {
  try {
    const user = await UserV2.findById(req.params.id)
      .populate("addresses")
      .select("-password");
...
exports.deleteUser = async (req, res) => {
  try {
    const user = await UserV2.findByIdAndDelete(req.params.id);
    ...
    await AddressV2.deleteMany({ userId: user._id });
...
exports.createAddress = async (req, res) => {
  try {
    ...
    const address = new AddressV2(addressData);
    await address.save();
...
exports.getAddressById = async (req, res) => {
  try {
    const address = await AddressV2.findOne({
      _id: req.params.id,
      userId: req.user.userId,
    });
...
exports.deleteAddress = async (req, res) => {
  try {
    const address = await AddressV2.findOneAndDelete({
      _id: req.params.id,
      userId: req.user.userId,
    });
